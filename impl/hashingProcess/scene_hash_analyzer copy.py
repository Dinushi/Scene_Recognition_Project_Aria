...
class Analyzer:
    ...
    def analyzeHDBetweenOriginalScenes(self, threshold = 30):
    ...
    def analyzeRobustness_withOriginalScene(self, threshold = 30):
        rotation_angle_array = ["0", "15", "30", "45", "60", "75", "90"]

        for scene_name, value_dict in self.test_hash_dictionary.items():
            fov_based_result_dic = {}
            fov_based_result_dic['130130'] = [0, 15, 30, 45, 60, 75, 90]
            fov_based_result_dic['120120'] = [0, 15, 30, 45, 60, 75, 90]
            fov_based_result_dic['110110'] = [0, 15, 30, 45, 60, 75, 90]
            fov_based_result_dic['100100'] = [0, 15, 30, 45, 60, 75, 90]
            fov_based_result_dic['9090'] = [0, 15, 30, 45, 60, 75, 90]

            original_hash_string = self.original_hash_key_dict[scene_name]
            for attack, test_hash_string in value_dict.items ():
                    fov_name = attack.split("_")[0][4:]
                    HD_array_of_angles = fov_based_result_dic[fov_name]
                    angle_index = rotation_angle_array.index(attack.split("_")[1])
                    hamming_dist_score = computeHammingDistancePercentage(original_hash_string, test_hash_string)
                    HD_array_of_angles[angle_index] = hamming_dist_score

                    fov_based_result_dic[fov_name] = HD_array_of_angles
        
    
            GraphicalVisualizer().plotMutipleGraphsInOnePlot(rotation_angle_array, fov_based_result_dic, scene_name)
